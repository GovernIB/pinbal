...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:fo="http://www.w3.org/1999/XSL/Format" version="2.0" xmlns:str="http://www.str.com" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:functx="http://www.functx.com" exclude-result-prefixes="str functx">
	...
	<!-- DG0001 -->
	<xsl:param name="ConsultaClaveClaveAdministrativa"/>
	<xsl:param name="ConsultaDiplomaNumDiploma"/>
	
	<xsl:param name="ConsultaDescripcion"/>
	<xsl:param name="ConsultaClaveAdministrativa"/>  <!-- DG0002 -->
	<xsl:param name="ConsultaClaveDescripcion"/>
	...
	<xsl:template match="//*[local-name()='DatosEspecificos']">
		...
		<xsl:variable name="datosNacimiento" select="$datosEspecificos/*[local-name()='Solicitud']/*[local-name()='DatosNacimiento']"/>
		<xsl:variable name="datosDireccion" select="$datosEspecificos/*[local-name()='Solicitud']/*[local-name()='Direccion']"/>
		...
		<fo:block border-style="solid" border="1px 0 0 0" text-indent="3mm" font-family="Arial,sans-serif" font-size="10pt" space-after="1mm">
			...
			<!-- TODO CAMBIAR EN UNA SOLA TABLA -->
				...
				<fo:table-body start-indent="0pt"> 
					<fo:table-row>
						<fo:table-cell padding="2pt" display-align="center">						
								<fo:block font-size="8pt" text-align="left" font-family="Arial,Helvetica,sans-serif" line-height="10pt" space-after.optimum="1pt" display-align="after" >
										<xsl:choose>
											<xsl:when test="$ConsultaClaveClaveAdministrativa != '' ">
												<fo:inline font-weight="bold">
													<xsl:text>Clave administrativa:     </xsl:text>
												</fo:inline>
												<xsl:value-of select="$ConsultaClaveClaveAdministrativa"/>
											</xsl:when>	
											
											<xsl:when test="$ConsultaClaveAdministrativa != '' ">
												<fo:inline font-weight="bold">
													<xsl:text>Clave administrativa:     </xsl:text>
												</fo:inline>
												<xsl:value-of select="$ConsultaClaveAdministrativa"/>
											</xsl:when>	
																					
											<xsl:when test="$ConsultaDiplomaNumDiploma != '' ">								
												<fo:inline font-weight="bold">
													<xsl:text>Num. Diploma:     </xsl:text>
												</fo:inline>
												<xsl:value-of select="$ConsultaDiplomaNumDiploma"/>
											</xsl:when>	
											
											<xsl:when test="$ConsultaDescripcion != '' ">								
												<fo:inline font-weight="bold">
													<xsl:text>Descripción:     </xsl:text>
												</fo:inline>
												<xsl:value-of select="$ConsultaDescripcion"/>
											</xsl:when>	
					...
					<fo:table-row>
						<fo:table-cell padding="2pt" display-align="center">						
								<fo:block font-size="8pt" text-align="left" font-family="Arial,Helvetica,sans-serif" line-height="10pt" space-after.optimum="1pt" display-align="after" >
											...
											<xsl:if test="$ConsultaClaveDescripcion != '' ">
												<fo:inline font-weight="bold">
													<xsl:text>Descripción:     </xsl:text>
												</fo:inline>
												<xsl:value-of select="$ConsultaClaveDescripcion"/>
											</xsl:if>	       
								</fo:block>													
						</fo:table-cell>							
					</fo:table-row>
