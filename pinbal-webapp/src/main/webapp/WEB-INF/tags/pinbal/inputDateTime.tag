...
<c:choose>
			...
			<div class="input-group date" style="width:100%">
				<c:choose>
					<c:when test="${myReadonly}">
						<form:input path="${campPath}" cssClass="form-control datetimepicker" id="${campPath}" disabled="${disabled}" data-idioma="${idioma}" readonly="${myReadonly}" />
					</c:when>
					<c:otherwise>
						<form:input path="${campPath}" cssClass="form-control datetimepicker" id="${campPath}" disabled="${disabled}" data-toggle="datetimepicker" data-idioma="${idioma}" />					
					</c:otherwise>
				</c:choose>
	...
	<c:otherwise>
		...
		<div class="input-group" style="width:100%">
			<c:choose>
				<c:when test="${myReadonly}">
					<form:input path="${campPath}" cssClass="form-control datetimepicker" id="${campPath}" placeholder="${campPlaceholder}" disabled="${disabled}" data-idioma="${idioma}" readonly="${myReadonly}"/>
				</c:when>
				<c:otherwise>
					<form:input path="${campPath}" cssClass="form-control datetimepicker" id="${campPath}" placeholder="${campPlaceholder}" disabled="${disabled}" data-toggle="datetimepicker" data-idioma="${idioma}"/>
				</c:otherwise>
			</c:choose>
