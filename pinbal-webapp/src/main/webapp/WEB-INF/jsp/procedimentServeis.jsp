	...
	<jmesa:tableModel
		...
		<jmesa:htmlTable>
			<jmesa:htmlRow>
				...
				<c:forEach var="procedimentServei" items="${procediment.serveisActius}">
					<c:if test="${registre.codi == procedimentServei}"><c:set var="trobat" value="${true}"/></c:if>
				</c:forEach>
				...
				<jmesa:htmlColumn property="CALCUL_actiu" titleKey="procediment.serveis.taula.columna.actiu">
					<c:if test="${trobat}"><i class="icon-ok"></i></c:if>
				</jmesa:htmlColumn>
				<jmesa:htmlColumn property="ACCIO_activar" title="&nbsp;" sortable="false">
					<c:choose>
						<c:when test="${not trobat}"><a href="servei/${registre.codi}/enable" class="btn"><i class="icon-ok"></i>&nbsp;<spring:message code="comu.boto.activar"/></a></c:when>
