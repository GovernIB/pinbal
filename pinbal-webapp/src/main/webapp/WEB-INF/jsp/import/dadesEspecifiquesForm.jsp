								...
								<c:when test="${empty dadaEspecifica.enumeracioValors}">
									<c:choose>
										...
										<c:when test="${camp.tipus == 'BOOLEA'}">
											...
											<input type="checkbox" id="${campId}" name="${campId}"<c:if test="${campChecked}"> checked="checked"</c:if><c:if test="${dadesEspecifiquesDisabled}"> disabled="disabled"</c:if>/>
											...
											<c:choose>
												...
												<c:otherwise>
													<div class="input-append">
														<input type="text" id="${campId}-file-helper" value="${serveiCommand.fitxerAjudaNom}" style="width:110%" class="form-control"/>
														<span id="${campId}-file-helper-clean" class="btn btn-default"><i class='icon-trash'></i></span>
														<span id="${campId}-file-helper-file" class="btn btn-default"><i class="fas fa-trash-alt"></i></span>
													</div>
