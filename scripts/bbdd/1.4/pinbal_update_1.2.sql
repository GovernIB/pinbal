...
ALTER TABLE PBL_SERVEI_CONFIG ADD FITXER_AJUDA_NOM VARCHAR2(255);
ALTER TABLE PBL_SERVEI_CONFIG ADD FITXER_AJUDA_MIME VARCHAR2(255);
ALTER TABLE PBL_SERVEI_CONFIG ADD FITXER_AJUDA_CONTINGUT BLOB;
ALTER TABLE PBL_SERVEI_CONFIG move lob (AJUDA) store as PBL_SERVEI_CONFIG_AJUDA_LOB (tablespace PINBAL_LOB);
ALTER TABLE PBL_SERVEI_CONFIG move lob (FITXER_AJUDA_CONTINGUT) store as PBL_SERVEI_CONFIG_FACONT_LOB (tablespace PINBAL_LOB);

ALTER TABLE PBL_CONSULTA ADD (
    CUSTODIA_URL VARCHAR2(255 CHAR),
    JUSTIFICANT_ESTAT NUMBER(10),
    JUSTIFICANT_ERROR VARCHAR2(1000 CHAR));
UPDATE PBL_CONSULTA SET JUSTIFICANT_ESTAT = 3 WHERE ESTAT = 3;
UPDATE PBL_CONSULTA SET JUSTIFICANT_ESTAT = 0 WHERE ESTAT = 1;
UPDATE PBL_CONSULTA SET JUSTIFICANT_ESTAT = 0 WHERE ESTAT = 2 AND CUSTODIAT = 0;
UPDATE PBL_CONSULTA SET JUSTIFICANT_ESTAT = 1 WHERE ESTAT = 2 AND CUSTODIAT = 1;
ALTER TABLE PBL_CONSULTA MODIFY JUSTIFICANT_ESTAT NOT NULL;

--Camp de procediment adicional per la taula de procediment-servei--
ALTER TABLE PBL_PROCEDIMENT_SERVEI ADD PROCEDIMENT_CODI VARCHAR2(20);

-- Error amb la custòdia de justificants
ALTER TABLE PBL_CONSULTA ADD CUSTODIA_ID VARCHAR2(255 CHAR);
