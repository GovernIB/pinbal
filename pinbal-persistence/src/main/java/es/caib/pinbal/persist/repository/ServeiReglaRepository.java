...
//	public List<ServeiRegla> findByServei(Servei servei);
	...
	@Query("from ServeiRegla r " +
			...
			"  and r.modificat in (es.caib.pinbal.logic.intf.dto.regles.ModificatEnum.ALGUN_CAMP, es.caib.pinbal.logic.intf.dto.regles.ModificatEnum.CAMPS) " +
			"order by r.ordre asc")
	public List<ServeiRegla> findReglesCamps(@Param("servei") Servei servei);

	@Query("from ServeiRegla r " +
			"where r.servei=:servei " +
			"  and r.modificat in (es.caib.pinbal.logic.intf.dto.regles.ModificatEnum.ALGUN_GRUP, es.caib.pinbal.logic.intf.dto.regles.ModificatEnum.GRUPS) " +
			"order by r.ordre asc")
...
//	public Long countByServeiId(@Param("serveiId") Long serveiId);
	...
	@Query("select c.id " +
			...
			"		   and rm.reglaId = r.id " +
			"		   and (r.modificat = es.caib.pinbal.logic.intf.dto.regles.ModificatEnum.CAMPS or r.modificat = es.caib.pinbal.logic.intf.dto.regles.ModificatEnum.ALGUN_CAMP)) " +
	...
	@Query("select g.id " +
			...
			"		   and rm.reglaId = r.id " +
			"		   and (r.modificat = es.caib.pinbal.logic.intf.dto.regles.ModificatEnum.GRUPS or r.modificat = es.caib.pinbal.logic.intf.dto.regles.ModificatEnum.ALGUN_GRUP)) " +
