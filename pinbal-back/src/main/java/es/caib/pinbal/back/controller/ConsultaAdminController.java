...
public class ConsultaAdminController extends BaseController {
	...
	public String justificantReintentar(
		...
		if (RolHelper.isRolActualAdministrador(request) || entitat != null) {
			try {
				...
				if (isHistoric(request)) {
					justificant = historicConsultaService.reintentarGeneracioJustificant(
							consultaId,
							false);
				} else {
	...
	private ConsultaFiltreCommand getCommandInstance(HttpServletRequest request) {
		ConsultaFiltreCommand command = (ConsultaFiltreCommand)RequestSessionHelper.obtenirObjecteSessio(
				...
				SESSION_ATTRIBUTE_FILTRE);
		if (command == null) {
			command = new ConsultaFiltreCommand(entitatService.findTopByTipus(EntitatDto.EntitatTipusDto.GOVERN).getId());
