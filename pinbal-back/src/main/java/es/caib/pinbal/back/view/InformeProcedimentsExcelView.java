 ...
 * @author Josep Gayà
 */
public class InformeProcedimentsExcelView extends AbstractHssfView implements MessageSourceAware {
	...
	protected void buildExcelDocument(
		...
		response.setHeader("Content-Disposition", "Inline; filename=informeProcediments.xls");
		List<InformeProcedimentDto> informeDades = (List<InformeProcedimentDto>)model.get("informeDades");
		HSSFSheet sheet = workbook.createSheet(
				getMessage(
						request,
						"informe.procediments.excel.fulla.titol"));
		...
		departamentStyle.setAlignment(HorizontalAlignment.LEFT);
		...
		departamentFont.setBold(true);
		departamentStyle.setFont(departamentFont);

		int rowIndex = 0;
		int totalProcedimentsEntitat = 0;
		...
		for (InformeProcedimentDto informeDada: informeDades) {
			if (entitatActual == null || !entitatActual.getId().equals(informeDada.getEntitat().getId())) {
				if (entitatActualCell != null) {
					entitatActualCell.setCellValue(entitatActual.getNom() + " (" + totalProcedimentsEntitat + ")");
					totalProcedimentsEntitat = 0;
				}
			...
			HSSFCell dadaActiuCell = filaDada.createCell(columnaInicial + 2);
			dadaActiuCell.setCellValue(
					getMissatgeSiNo(
							request,
							informeDada.isActiu()));
			totalProcedimentsEntitat++;
			subtotalDepartament++;
		}
		if (entitatActualCell != null) {
			entitatActualCell.setCellValue(entitatActual.getNom() + " (" + totalProcedimentsEntitat + ")");
	...
	private void insertarFilaCapsalera(
		...
		codiCell.setCellValue(
				getMessage(
						request,
						"informe.procediments.excel.columna.codi"));
		HSSFCell nomCell = titolsColumna.createCell(columnaInicial + 1);
		nomCell.setCellStyle(style);
		nomCell.setCellValue(
				getMessage(
						request,
						"informe.procediments.excel.columna.nom"));
		HSSFCell actiuCell = titolsColumna.createCell(columnaInicial + 2);
		actiuCell.setCellStyle(style);
		actiuCell.setCellValue(
				getMessage(
						request,
						"informe.procediments.excel.columna.actiu"));
	}
	...
	private String getMissatgeSiNo(
			HttpServletRequest request,
			boolean valor) {
		return getMessage(
				request,
				(valor) ? "comu.si" : "comu.no");
	}
