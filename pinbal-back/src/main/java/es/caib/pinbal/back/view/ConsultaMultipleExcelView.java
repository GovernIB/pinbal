 ...
 * @author Josep Gayà
 */
public class ConsultaMultipleExcelView extends AbstractHssfView implements MessageSourceAware {
	...
	private int taulaConsulta(
		...
		titolStyle.setAlignment(HorizontalAlignment.CENTER);
		titolStyle.setVerticalAlignment(VerticalAlignment.CENTER);
		HSSFFont titolFont = workbook.createFont();
		titolFont.setFontName("Arial");
		titolFont.setFontHeightInPoints((short)14);
		titolFont.setBold(true);
		...
		contingutTextStyle.setAlignment(HorizontalAlignment.LEFT);
		contingutTextStyle.setVerticalAlignment(VerticalAlignment.TOP);
		...
		contingutDataStyle.setAlignment(HorizontalAlignment.LEFT);
		contingutDataStyle.setVerticalAlignment(VerticalAlignment.TOP);
		...
		capsaleraCell.setCellValue(
				getMessage(
						request,
						"consulta.list.taula.capsalera.multiple.titol"));
		...
		HSSFCell derCellColumnName = capsalera1.createCell(indexColumna);
		derCellColumnName.setCellStyle(capsaleraStyle);
		derCellColumnName.setCellValue(
				getMessage(
						request,
						"consulta.list.taula.der"));
		...
		if (consultes != null) {
			...
			for (ConsultaDto consulta: consultes) {
				...
				HSSFCell derCell = filaActual.createCell(indexColumna);
				if (consulta.getDataEsperadaResposta() != null) {
					derCell.setCellValue(sdf.format(consulta.getDataEsperadaResposta()));
				} else {
					derCell.setCellValue("");
				}
				derCell.setCellStyle(contingutDataStyle);
