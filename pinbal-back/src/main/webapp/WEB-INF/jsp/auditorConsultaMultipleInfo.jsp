	...
	<div class="well">
		...
		<div id="dadesPeticio" class="collapse in">
			<c:if test="${consulta.hiHaPeticio}">
				...
				<a class="btn btn-default pull-right" href="<c:url value="/modal/auditor/${consulta.id}/xmlPeticio"/>" onclick="${initModalXml}">
					...
					<tbody>
						<c:forEach var="filla" items="${filles}">
							<tr>
								...
								<c:if test="${consulta.hiHaPeticio}">
									<td>
										<a class="btn btn-default pull-right" href="<c:url value="/modal/auditor/${filla.id}/xmlPeticio"/>" onclick="${initModalXml}">
											<i class="fas fa-info-circle"></i>
											<spring:message code="consulta.info.veure.xml"/>
										</a>
									</td>
									<td>
										<a class="btn btn-default pull-right" href="<c:url value="/modal/auditor/${filla.id}/xmlResposta"/>" onclick="${initModalXml}">
											<i class="fas fa-info-circle"></i>
											<spring:message code="consulta.info.veure.xml"/>
										</a>
									</td>
								...
								<td>
									<a href="<c:url value="/modal/auditor/${filla.id}?multiple=true"/>" class="btn btn-default"><i class="fas fa-search-plus"></i>&nbsp;<spring:message code="consulta.list.taula.detalls"/></a>
								</td>
								<c:if test="${consulta.estatTramitada}">
									<td>
	...
	<div class="well well-sm">
		<div class="row">
			...
			<div class="col-xs-2 mt15 pull-right">
				<c:set var="initModalXml">initModalXml(this);return false</c:set>
				<a class="btn btn-default pull-right" href="<c:url value="/modal/auditor/${consulta.id}/xmlResposta"/>" onclick="${initModalXml}"><i class="fas fa-info-circle"></i> <spring:message code="consulta.info.veure.xml"/></a>
			</div>
		</div>
...
$('#dadesResposta').on('shown', function () {
	...
	<div id="modal-botons" class="well">
		<a href="<c:url value="/auditor"/>" class="btn btn-default" data-modal-cancel="true"><spring:message code="comu.boto.tancar"/></a>
	</div>
