	...
	<div class="tab-content">
					...
					<fieldset>
						<legend><spring:message code="consulta.form.dades.generiques"/>
							<c:if test="${consulta.hiHaPeticio}">
								...
								<a class="btn btn-default pull-right" style="top: -4px; position: relative;" href="<c:url value="/modal/superauditor/${consulta.id}/xmlPeticio"/>" onclick="${initModalXml}">
		...
		<div class="tab-pane" id="dadesRespostaTab" role="tabpanel">
			<c:if test="${consulta.hiHaResposta}">
				<div class="well well-sm">
					<fieldset>
						<legend><spring:message code="consulta.info.resposta.dades"/>
							<c:set var="initModalXml">initModalXml(this);return false</c:set>
							<a class="btn btn-default pull-right" style="top: -4px; position: relative;" href="<c:url value="/modal/superauditor/${consulta.id}/xmlResposta"/>" onclick="${initModalXml}">
								<i class="fas fa-info-circle"></i> <spring:message code="consulta.info.veure.xml"/>
							</a>
						</legend>
						<div class="clearfix legend-margin-bottom"></div>
	...
	<div id="modal-botons" class="well">
		<c:choose>
			<c:when test="${multiple and not empty consulta.pareId}">
				<a href="<c:url value="/modal/superauditor/${consulta.pareId}"/>" class="btn btn-default"><span class="fa fa-arrow-left"></span>&nbsp;<spring:message code="comu.boto.tornar"/></a>
			</c:when>
			<c:otherwise>
